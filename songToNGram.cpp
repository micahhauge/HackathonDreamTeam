...
const int LAST_FILE_INDEX = 26; // 26 Mendelson files
...
int main(){
	...
	for (int fileTag = 0; fileTag < LAST_FILE_INDEX; fileTag++){
	
		is.open("./midiFiles/out_" + to_string(fileTag) + ".txt");
		assert(is);
		...
		ofstream outfile;
		outfile.open("./nGramData/out_" + to_string(fileTag) + ".txt");
